...
### 2026-07-17 - Milestone 12 scope conflict identified and resolved with Raymond
The first Milestone 12 draft proposed a fully read-only, non-persisted design (matching Milestone 11's pattern) that conflicted with the frozen `Phase 2.5 Data Model` and `Phase 3 Implementation Plan`, which both specify a persisted `morning_brief_items` record type with a review/correction lifecycle. Raymond confirmed the frozen documents remain the source of truth: "read-only" applies to source records only, not the Morning Brief itself. The proposal was revised to a persisted, request-driven design through two further rounds of Raymond's questions (batch identity mechanism, whether `corrected_section`/`reviewed_at` were truly necessary, whether the 7-item cap should downgrade stored `importance`). Final decisions: `brief_batch_id` (a `randomUUID()`, not a timestamp) is the batch identity key; `corrected_section` and `reviewed_at` were dropped as unnecessary for any stated V1 behavior; `confidence` stays in the schema but stays fixed and unused; `importance` is fixed per section and never varies by item count or display position; the 7-item Requires Raymond cap is a frontend-only display choice.

### 2026-07-17 - Milestone 12 approved and completed as a persisted Morning Brief
Milestone 12 added the `morning_brief_items` table (16 columns: 12 from the frozen model, 4 new) and four endpoints: `POST /morning-brief` (generate + persist), `GET /morning-brief/latest`, `GET /morning-brief/history`, `PATCH /morning-brief-items/:id` (accept/dismiss/correct). Generation reads only Tasks, Projects, Raw Captures, and Review Later Resources; review actions write only to `morning_brief_items`; no source record is ever mutated, verified by a test snapshotting all seven source tables before and after every endpoint call. The optional AI narrative reuses the Milestone 10.1/11 OpenAI provider plumbing, can only populate a per-item `ai_narrative` field after deterministic selection is already final, and sends no internal IDs externally (items are correlated back by array position, not database id). Testing caught and fixed a real bug before shipping: two same-millisecond generations could tie on `generated_at`, and the original `ORDER BY ... id DESC` tiebreaker picked a random winner since ids are UUIDs; fixed by ordering on SQLite's monotonic `rowid` instead. Live OpenAI verification remains deferred to the bundled end-of-Version-1 smoke test alongside Milestone 10.1 and 11.
