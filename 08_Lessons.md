...
- "Read-only" is ambiguous shorthand - it can mean "the feature doesn't persist anything" (Milestone 11) or "the feature doesn't mutate source records but does persist its own output" (Milestone 12, per the frozen Phase 2.5 model). Naming which one explicitly, and checking against the frozen model before assuming the narrower reading, avoids building the wrong shape.
- A timestamp string (`generated_at`) is a display/ordering field, not an identity key - two events can share a millisecond. A `randomUUID()` batch identifier removes the collision class entirely instead of just making it unlikely; when a timestamp tiebreaker is unavoidable elsewhere (e.g. `ORDER BY`), use a monotonic value like SQLite's `rowid`, never another random field like an id, since ties on the primary sort key make the secondary sort the real tiebreaker.
- When a required schema field doesn't need real logic yet (e.g. `confidence` fixed at `1.0` for deterministic items), keep the field but make its inertness explicit and tested - schema-complete now, meaningfully populated later, is cheaper than adding the column in a future migration.
- Fields that describe an item (like `importance`) should never be derived from the item's position in a list or how many other items exist - display-driven concerns (like a 7-item cap) belong entirely in the frontend, keeping stored data stable regardless of how it's later shown.
